...
For what it's worth, Google does maintain an [Eclipse Plugin](https://developers.google.com/eclipse/docs/apps_script) which handles importing and exporting of [standalone scripts](https://developers.google.com/apps-script/guides/standalone) such as this one and enables Git integration. Scripts are hosted separately from other Drive files, so a project with multiple scripts is represented by a pointer file in your Drive, of which [`csv2sheet_scripts.gscript`](https://github.com/unendin/csv2sheet/blob/master/csv2sheet_scripts.gscript) is an instance. Google generates a new `gscript` file with your own URL, ID, and username once you begin using or editing the scripts. 
